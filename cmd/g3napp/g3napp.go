...
func StartG3nApp(initialCam math32.Vector3, width, height int, holder runner.ResultQueueHolder, done chan<- bool) {
	...
	go func() {
		...
		g3nApp.flyCam = flycam.NewFlyCam(window.Get().(*window.GlfwWindow), g3nApp.scene, width, height)  // TODO test in struct initializer
...
//
func addBody(bri bodyrender.Renderable) *graphic.Mesh {
	var mesh *graphic.Mesh
	if bri.IsSun() {
		...
		pl := light.NewPoint(xlatColor(globals.White), 500)
		pl.SetLinearDecay(.0002)
		pl.SetQuadraticDecay(.0002)
		pl.SetPosition(bri.X(), bri.Y(), bri.Z())
		g3nApp.scene.Add(pl)
