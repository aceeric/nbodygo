...
Defining this variable causes the instrumentation package to start the Prometheus HTTP server, and causes
all instrumentation calls to actually instantiate and register Prometheus metric collectors. If you don't
specify that environment variable, no HTTP server is started, and all the metric calls wind up at
...
unless needed, since most of the time you want the app to run with as high a throughput as possible. Granted,
there probably isn't a use case for this when instrumenting a production service, but for development
